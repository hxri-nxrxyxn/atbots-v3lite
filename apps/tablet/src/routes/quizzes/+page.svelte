<script lang="ts">
	...
	import { CheckCircle2, CircleStop, RotateCcw, Volume2, XCircle } from '@lucide/svelte';
...
<div class="flex h-full flex-col">
	<PageHeader
		title={selectedQuiz ? selectedQuiz.title : 'Quizzes'}
		subtitle={selectedQuiz ? `Category: ${selectedQuiz.category}` : 'Offline knowledge challenge'}
		onBack={selectedQuiz ? backToCatalog : undefined}
		backHref={resolve('/')}
	>
		{#snippet actions()}
			{#if speaking}
	...
	>
		{#if !selectedQuiz}
			...
			<div class="space-y-6 flex-1 overflow-y-auto" use:staggerIn>
				<div class="flex items-center gap-5 border-b border-border/70 pb-5">
					...
					<div>
						<h2 class="text-xl font-bold tracking-tight text-foreground">Quiz Library</h2>
		...
		{:else if quizFinished}
			<!-- Completion Summary -->
			<div class="flex flex-1 flex-col items-center justify-center" use:fadeIn>
				<div class="border-border bg-card/40 w-full rounded-2xl border p-8 text-center space-y-6">
					...
					<div>
						<h2 class="text-2xl font-bold tracking-tight text-foreground">Quiz Completed</h2>
					...
					<div class="flex gap-3 pt-2">
						<Button
							variant="outline"
							class="flex-1 gap-1.5 text-xs font-medium"
							onclick={resetQuiz}
						>
		...
		{:else if currentQuestion}
			...
			<div class="flex flex-1 flex-col justify-between gap-5" use:fadeIn>
				<div class="space-y-5">
					...
					<div class="flex items-center gap-5 border-b border-border/70 pb-5">
						...
						<h2 class="text-base font-semibold tracking-tight leading-snug">
